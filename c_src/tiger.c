...
static inline uint8_t * tigerhash_hash(tigerhash *th, const void *data, size_t len) {
  tigerhash_update(th, data, len);
  tigerhash_finalize(th);
  return tigerhash_result(th);
}

static inline void tigerhash_str(char str[TIGER_STR_BYTES], uint8_t *hash) {
  char *p = str;
  int i = 0;
  for(; i < TIGER_BYTES; i++) {
...
void tiger_str(char str[TIGER_STR_BYTES], const void *data, size_t len) {
  tigerhash_str(str, tiger(data, len));
  
}

void tiger2_str(char str[TIGER_STR_BYTES], const void *data, size_t len) {
  tigerhash_str(str, tiger2(data, len));  
}
